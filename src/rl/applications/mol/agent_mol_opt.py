...
from src.rl.applications.mol import hyp, utils
...
class Agent(object):
    ...
    def update_params(self, batch_size, gamma, polyak, approx=False, triple=False, approx_reward=None,
        ...
        states_smiles, values, rewards, next_states_smiles, dones = self.replay_buffer.sample(batch_size)
        #print(states_smiles[0])
        states = [np.append(utils.get_fingerprint(states_smiles[i, 0],
        ...
        for i in range(len(next_states_smiles)):
            arr = [np.append(utils.get_fingerprint(next_states_smiles[i][j, 0], hyp.fingerprint_length,
        ...
        if approx:
            ...
            for i in range(len(states_smiles)):
                if triple:
                    rewards.append(approx_reward(states_smiles[i, 0], values[i][0], values[i][1],
